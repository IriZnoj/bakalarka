...
void strom::vlozpom(const slovo &x,uzel*&p){
	if(p==NULL){
		p=new uzel;
		p->klic=x;
		p->levy=NULL;
...
void strom::zapisvporadipom(uzel*&p,string nazev){
	if(p!=NULL){
		zapisvporadipom(p->levy,nazev);
		p->klic.dosouboru(nazev);
		zapisvporadipom(p->pravy,nazev);
	}
}
void strom::zapisvporadi(string nazev){
	zapisvporadipom(koren,nazev);
}